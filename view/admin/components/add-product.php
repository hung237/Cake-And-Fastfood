...
<?php
    ...
    if(isset($_POST['add-product'])){
        ...
        if(!empty(trim($name_product)) && !empty(trim($kind)) && !empty(trim($price)) && !empty(trim($des)) && !empty(trim($image))){
            ...
            while($checkUrl!=1);
            addItem($name_product,$price,$des,$kind,$newimage);
            move_uploaded_file($_FILES['image']['tmp_name'],$target_file);
            echo "<script>
                alert('Thêm sản phẩm thành công');
            </script>";
            echo "<meta http-equiv='refresh' content='0;url=index.php?act=addproduct'>";
        }
        else{
...
<div class="right col10">
    <div class="box-update">
        ...
        <div class="form margin-top-40px">
            <form action="index.php?act=addproduct" method="post" class="box-update" enctype="multipart/form-data">
